...
describe("style-base", () => {
    ...
    //  具体测试
    function tmpFunc(sm: ILinkManager & IScope, index: number) {
        //  基础测试
        ...
        const containner = globalThis.document.getElementById(LINK_CONTAINER_ID)!;
    ...
    test("global", () => tmpFunc(link as ILinkManager & IScope, 1));
    test("scope-null", () => tmpFunc(useLink(), 2));
    test("scope-Object", () => tmpFunc(useLink(), 3));
});
...
describe("style-config", () => {
    ...
    test("测试", () => {
        ...
        //  全局配置
        const sm1: ILinkManager = link;
        configLink({ theme: "CD", container: container1, origin: "http://snail.dev", version })
    ...
    test("onRegister", () => {
        ...
        linkMap.set("file.css", "/xxxx/s/x/da.css");
        globalThis.document.body.appendChild(container1);
        link.register("file.css", { file: "http://file.css", theme: "CD" });
...
//  每次执行完后，重置全局配置
afterEach(() => {
    configLink({ theme: undefined, container: undefined, origin: undefined, version: undefined });
    link.theme(new Date().toString());
});
//  还原状态
afterAll(() => {
    //@ts-ignore
