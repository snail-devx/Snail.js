...
<template>
    <div class="node-panel" v-if="node.disabled != true">
        ...
        <div class="children" v-if="hasChildrenRef" ref="children">
            <TreeNode2 v-for="child in node.children" :key="newId()" :node="child" :parent="node" :level="nextLevel"
...
import { Tree2NodeEvents, Tree2NodeOptions } from "./tree2-model";
...
// *****************************************   👉  组件定义    *****************************************
//  1、props、data
const { node, parent, level, extend = {} } = defineProps<Tree2NodeOptions<any>>();
throwIfTrue(level > 10, "tree node level cannot exceed 10.");
const emits = defineEmits<Tree2NodeEvents<any>>();
...
//  2、可选配置选项
defineOptions({ name: "TreeNode2", inheritAttrs: true, });
