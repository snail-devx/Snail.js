...
describe('parseTime', () => {
    ...
    it('超出范围的小时应返回 undefined', () => {
        const result = parseTime('25:30:45');
        expect(result).toBeUndefined();
    });

    it('超出范围的分钟应返回 undefined', () => {
        const result = parseTime('12:60:45');
        expect(result).toBeUndefined();
    });
    ...
    it('负数小时应返回 undefined', () => {
        const result = parseTime('-1:30:45');
        expect(result).toBeUndefined();
    });

    it('边界值 0:0:0 应有效', () => {
        const result = parseTime('0:0:0');
...
describe('correctTime', () => {
    ...
    it('undefined 输入应返回 undefined', () => {
        const result = correctTime(undefined as any);
        expect(result).toBeUndefined();
    });
...
describe('useTime   HH 格式', () => {
    const manager = useTime({ format: 'HH' });
...
describe('useTime   HH:mm 格式', () => {
    const manager = useTime({ format: 'HH:mm' });
...
describe('useTime   默认配置选项', () => {
    ...
    it('自定义 min 应使用传入值', () => {
        const manager = useTime({ min: { hour: 8, minute: 0, second: 0 } });
    ...
    it('自定义 max 应使用传入值', () => {
        const manager = useTime({ max: { hour: 20, minute: 0, second: 0 } });
...
describe('useTime   边界情况测试', () => {
    ...
    it('应处理 NaN 值', () => {
        const result = parseTime('a:b:c');
        expect(result).toBeUndefined();
    });

    it('应处理多余的分隔符', () => {
        const result = parseTime('12:30:45:99');
        expect(result?.hour).toBe(12);
        expect(result?.minute).toBe(30);
