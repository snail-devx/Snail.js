...
<template>
  <div class="setting-item">
    ...
    <Number class="item-detail" v-else :placeholder="placeholder" :min-value="minValue" :max-value="maxValue"
      :clamp="'clamp'" :precision="precision" :controls="'right'" :step="1" v-model="valueRef"
      @change="(newValue, oldValue) => emits('change', newValue, oldValue)" />
    <p class="item-error ellipsis" v-if="isStringNotEmpty(error)" v-text="error" />
  </div>
</template>
